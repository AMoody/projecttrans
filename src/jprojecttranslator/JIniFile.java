...
public class JIniFile extends ArrayList {
  ...
  public JIniFile(String FileName) {
    ...
    // If the filename given was not absolute then try to find the folder where this class exists and assume the ini file is there.
    if(!tempFileName.isAbsolute()) {
        this.userFileName = new File(getProjectFolder(), tempFileName.toString());
    }else{
        this.userFileName = tempFileName;
    }
  ...
  public boolean ReadBool(String Section, String key, boolean defaultValue) {
    ...
    if (ValuePosition(Section, key) > 0) {
      int strLen = key.length()+1;
      intvalue = Integer.parseInt(get(ValuePosition(Section, key)).toString().substring(strLen, get(ValuePosition(Section, key)).toString().length()));
      if (intvalue == 1) {
          value = true;
      } else {
          value = false;
  ...
  /** GetFileNameString returns the complete file name and path as a string
   * this allows you to query the path which may not have been set in the constructor
   */
  public String GetFileNameString() {
      return this.userFileName.toString();
  } 
  
  /** GetFileName returns the complete file name and path 
   * this allows you to query the path which may not have been set in the constructor
   */
  ...
  /** MergeFile is used to merge the contents of one file to another.
   * Added by Arthur Moody 9th Mar 2003
   */
  public void MergeFile(JIniFile sourceFile) {
   ArrayList sourceSections = sourceFile.ReadSections();
   String sectionName, keyName, value;
   ArrayList sourceKeys;
   for (int i=0; i < sourceSections.size(); i++) {
       // Loop through each section in the source file
       sectionName = sourceSections.get(i).toString();
       sourceKeys = sourceFile.ReadSection(sectionName);
       //This returns the keys of this section
       for (int j=0; j < sourceKeys.size(); j++) {
           // Loop through the keys copying the value to the detination file
           keyName = sourceKeys.get(j).toString();
           value = sourceFile.ReadString(sectionName,keyName,"");
           this.WriteString(sectionName,keyName,value);
